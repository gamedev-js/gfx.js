...
function _commitDepthStates(gl, cur, next) {
  ...
  if (next.depthTest === false) {
    if (next.depthWrite) {
      next.depthTest = true;
      next.depthFunc = enums.DEPTH_FUNC_ALWAYS;
...
function _commitStencilStates(gl, cur, next) {
  ...
  // commit stencil function and stencil operation states
  if (next.stencilSep) {
  ...
  clear(opts) {
    ...
    if (opts.depth !== undefined) {
      if (this._current.depthTest === false) {
        ...
        if (this._current.depthFunc !== enums.DEPTH_FUNC_ALWAYS) {
