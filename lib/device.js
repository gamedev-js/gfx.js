...
function _commitStencilStates(gl, cur, next) {
  ...
  if(next.stencilSep) {
    ...
    gl.stencilFuncSeparate(gl.BACK, next.stencilFuncFront, next.stencilRefFront, next.stencilMaskFront);
    gl.stencilMaskSeparate(gl.BACK, next.stencilWriteMaskFront);
    gl.stencilOpSeparate(gl.BACK, next.stencilFailOpFront, next.stencilzFailOpFront, next.stencilZPassOpFront);
  } else {
...
export default class Device {
  ...
  _initCaps() {
    const gl = this._gl;
    const extDrawBuffers = this.ext('EXT_draw_buffers');

    this._caps.maxVertexTextures = gl.getParameter(gl.MAX_VERTEX_TEXTURE_IMAGE_UNITS);
    this._caps.maxFragUniforms = gl.getParameter(gl.MAX_FRAGMENT_UNIFORM_VECTORS);
    this._caps.maxTextureUnits = gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS);

    this._caps.maxDrawBuffers = extDrawBuffers ? gl.getParameter(extDrawBuffers.MAX_COLOR_ATTACHMENTS_EXT) : 1;
    this._caps.maxColorAttachments = extDrawBuffers ? gl.getParameter(extDrawBuffers.MAX_COLOR_ATTACHMENTS_EXT) : 1;
  }
