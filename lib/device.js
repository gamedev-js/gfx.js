...
export default class Device {
   ...
   * @param {Number} slot
   */
  setTextureArray(name, textures, slot) {
    if (slot + textures.length >= this._caps.maxTextureUnits) {
      console.warn(`Can not set ${textures.length} textures ${name} from stage ${slot}, max texture exceed: ${this._caps.maxTextureUnits}`);
      return;
    }
    let slots = new Int32Array(textures.length);
    for (let index = 0; index < textures.length; ++index) {
      this._next.textureUnits[slot + index] = textures[index];
