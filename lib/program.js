...
function parseErrorLog(errLog) {
  ...
  errLog.split('\n').forEach(function (errMsg) {
    ...
    let parts = /^ERROR\:\s+(\d+)\:(\d+)\:\s*(.*)$/.exec(errMsg);
    if (parts) {
      result.push({
        fileID: parts[1] | 0,
        line: parts[2] | 0,
...
export default class Program {
  ...
  link() {
    ...
    if (!gl.getShaderParameter(vertShader, gl.COMPILE_STATUS)) {
      let vertexShaderErrors = parseErrorLog(gl.getShaderInfoLog(vertShader));
      vertexShaderErrors.forEach(error => {
        errors.push(error);
        console.error(`Failed to compile vertex shader:: fileID ${error.fileID}, line ${error.line}, message: ${error.message}`);
      });
      failed = true;
    }

    if (!gl.getShaderParameter(fragShader, gl.COMPILE_STATUS)) {
      let fragmentShaderErrors = parseErrorLog(gl.getShaderInfoLog(fragShader));
      fragmentShaderErrors.forEach(error => {
        errors.push(error);
        console.error(`Failed to compile fragment shader:: fileID ${error.fileID}, line ${error.line}, message: ${error.message}`);
      });
      failed = true;
    }
